...
var findEnvVarName = function(names) {
	if (_.isArray(names)) names = _.toArray(arguments);
	return _(names).find(function(name) {
		return _(process.env).has(name);
	});
};
...
exports.execScript = function(script, options, callback) {
	...
	var pathVarName = findEnvVarName('PATH', 'Path', 'path');
