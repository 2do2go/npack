...
var separatorEndedPathRegExp = new RegExp(path.sep + '$');
...
exports.execScript = function(script, options, callback) {
	...
	var npackDir = options.dir;
	npackDir = separatorEndedPathRegExp.test(npackDir) ?
		npackDir.substr(0, npackDir.length - 1) : npackDir;

	var env = _(options.env || {}).extend(process.env, {
