...
exports.syncWithLockfile = function(options, callback) {
	Steppy(
		function() {
			fse.pathExists(
				path.join(options.cwd, 'npm-shrinkwrap.json'),
				this.slot()
			);
		},
		function(err, lockfileExists) {
			if (!lockfileExists) {
				throw new Error('Lockfile is not found');
			}
