...
exports.install = function(options, callback) {
	...
	Steppy(
		...
		function(err, pkgInfos, _newPkgInfo) {
			...
			fsUtils.exists(newPkgNodeModulesPath, this.slot());

			// remove tar.gz if exists
			fse.remove(newPkgTarGzPath, this.slot());
		},
		function(err, nodeModulesExists, newPkgNodeModulesExists) {
			// copy node_modules to new package path
			if (nodeModulesExists && !newPkgNodeModulesExists) {
		...
		function() {
			...
			if (newPkgInfo.hooks.preinstall) {
				...
				processUtils.exec(newPkgInfo.hooks.preinstall, {
					cwd: newPkgSubPath,
					log: options.log
				}, this.slot());
			} else {
		...
		function() {
			...
			if (newPkgInfo.hooks.postinstall) {
				...
				processUtils.exec(newPkgInfo.hooks.postinstall, {
					cwd: targetPkgPath,
					log: options.log
				}, this.slot());
			} else {
	...
	Steppy(
		...
		function(err, pkgInfo) {
			...
			if (pkgInfo.hooks.preuse) {
				...
				processUtils.exec(pkgInfo.hooks.preuse, {
					cwd: pkgInfo.path,
					log: options.log
				}, this.slot());
			} else {
		...
		function(err, pkgInfo) {
			...
			if (pkgInfo.hooks.postuse) {
				...
				processUtils.exec(pkgInfo.hooks.postuse, {
					cwd: pkgInfo.path,
					log: options.log
				}, this.slot());
			} else {
	...
	Steppy(
		...
		function(err, pkgInfo) {
			...
			if (pkgInfo.hooks.preuninstall) {
				...
				processUtils.exec(pkgInfo.hooks.preuninstall, {
					cwd: pkgInfo.path,
					log: options.log
				}, this.slot());
			} else {
		...
		function(err, pkgInfo) {
			...
			if (pkgInfo.hooks.postuninstall) {
				...
				processUtils.exec(pkgInfo.hooks.postuninstall, {
					cwd: pkgInfo.path,
					log: options.log
				}, this.slot());
			} else {
