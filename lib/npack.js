...
exports.install = function(options, callback) {
	...
	var getPkgFromLocalSource = function(src, callback) {
		Steppy(
			...
			function(err, stat) {
				if (stat.isDirectory()) {
					fse.copy(src, tmpPkgSubPath, this.slot());
				} else if (stat.isFile()) {
					fse.copy(src, tmpPkgTarGzPath, this.slot());
				} else {
	...
	var log = function() {
		if (options.log) logger.log.apply(null, arguments);
	};
	...
	Steppy(
		function() {
			...
			log('Install new package from "%s"', options.src);
		...
		function() {
			...
			fsUtils.exists(tmpPkgTarGzPath, this.slot());
		},
		...
		function() {
			...
			log('Read package info from "%s"', tmpPkgSubPath);
		...
		function(err, currentNodeModulesExists, tmpPkgNodeModulesExists) {
			...
			if (currentNodeModulesExists && !tmpPkgNodeModulesExists) {
				log('Copy current package node_modules to temp package');
		...
		function() {
			log('Sync temp package npm dependencies');
		...
		function() {
			...
			if (tmpPkgInfo.hooks.preinstall) {
				log('Exec "preinstall" hook "%s"', tmpPkgInfo.hooks.preinstall);
		...
		function() {
			log('Move package to "%s"', newPkgPath);
		...
		function() {
			if (options.use) {
				log('Switch to new package "%s"', newPkgName);
		...
		function() {
			...
			if (tmpPkgInfo.hooks.postinstall) {
				log('Exec "postinstall" hook "%s"', tmpPkgInfo.hooks.postinstall);
...
exports.use = function(options, callback) {
	...
	Steppy(
		function() {
			...
			log('Set package "%s" as current', options.name);
		...
		function(err, pkgInfo) {
			log('Package successfully set as current');
...
exports.getInfo = function(options, callback) {
	Steppy(
		...
		function(err, pkgInfo) {
			if (!pkgInfo) return callback(null, null);
...
exports.uninstall = function(options, callback) {
	...
	Steppy(
		function() {
			...
			log('Uninstall package "%s"', options.name);
		...
		function() {
			log('Package successfully uninstalled');
			this.pass(null);
		},
		callback
	);
...
exports.clean = function(options, callback) {
	...
	Steppy(
		...
		function(err, pkgInfos) {
			...
			if (pkgInfos.length) {
				...
				_(pkgInfos).each(function(pkgInfo) {
					...
					exports.uninstall(uninstallOptions, group.slot());
				});
			} else {
		...
		function() {
			log('Packages successfully cleaned');
			this.pass(null);
		},
		callback
	);
