	...
	Steppy(
		...
		function() {
			if (/^https?:/.test(options.src)) {
				...
				logger.log('Download from remote source "%s"', options.src);
				...
				logger.log('Copy from local source "%s"', options.src);
		...
		function(err, tarGzExists) {
			// unpack tar.gz
			if (tarGzExists) {
				logger.log('Package is a tarball archive, extract it');
		...
		function(err, pkgInfos, _tmpPkgInfo) {
			...
			if (tmpPkgInfo.compatibility) {
				if (!semver.satisfies(options.version, tmpPkgInfo.compatibility)) {
					throw new Error(
						'Current npack version "' + options.version + '" ' +
	...
	Steppy(
		...
		function(err, pkgInfo) {
			...
			if (pkgInfo.compatibility) {
				if (!semver.satisfies(options.version, pkgInfo.compatibility)) {
					throw new Error(
						'Current npack version "' + options.version + '" ' +
						'doesn\'t satisfy version required by package: "' +
