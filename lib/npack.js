	...
	Steppy(
		...
		function() {
			...
			if (newPkgInfo.hooks.preinstall) {
				...
				processUtils.execScript(newPkgInfo.hooks.preinstall, {
					cwd: newPkgSubPath,
					dir: options.dir,
					cwdNodeModules: true,
					log: options.log
		...
		function() {
			log('Create link to node_modules from package');

			fse.symlink(
				nodeModulesPath,
				path.join(targetPkgPath, 'node_modules'),
				'dir',
				this.slot()
			);
		},
		...
		function() {
			...
			if (newPkgInfo.hooks.postinstall) {
				...
				processUtils.execScript(newPkgInfo.hooks.postinstall, {
					cwd: path.join(options.dir, 'package'),
					dir: options.dir,
					log: options.log
				}, this.slot());
			} else {
